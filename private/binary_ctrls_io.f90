...
! ***********************************************************************
      ...
      subroutine do_one_binary_setup(b, inlist, ierr)
         ...
         if (len_trim(b% extra_binary_terminal_output_file) /= 0) then
            b% extra_binary_terminal_iounit = alloc_iounit(ierr)
            if (ierr /= 0) then
               write(*,*) 'failed to allocate io unit for ' // trim(b% extra_binary_terminal_output_file)
               return
            end if
            open(unit=b% extra_binary_terminal_iounit, file=trim(b% extra_binary_terminal_output_file), &
                     action='write', status='replace',iostat=ierr)
            if (ierr /= 0) then
               write(*,*) 'failed to open ' // trim(b% extra_binary_terminal_output_file)
               call free_iounit(b% extra_binary_terminal_iounit)
               return
            end if
         end if
      ...
      subroutine store_binary_controls(b, ierr)
         ...
         ! mass transfer controls
         b% rlobe_scheme = rlobe_scheme
         b% ignore_rlof = ignore_rlof
         ...
         b% limit_retention_by_vcrit = limit_retention_by_vcrit
      ...
      subroutine set_binary_controls_for_writing(b, ierr)
         ...
         ! mass transfer controls
         rlobe_scheme = b% rlobe_scheme
         ignore_rlof = b% ignore_rlof
         ...
         limit_retention_by_vcrit = b% limit_retention_by_vcrit
