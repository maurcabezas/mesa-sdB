...
! ***********************************************************************
    ...
    subroutine edot_enhancement_Isotropic(binary_id, ierr)
       ...
       b% e1 = (b% eccentricity + cos(b% theta_co)) 
       b% e2 = 2 * cos(b% theta_co) + b% eccentricity + b% eccentricity * cos(b% theta_co)**2
       b% e3 = b% eccentricity * sin(b% theta_co)**2
...
!               ((b% m(b% a_i)**2 / b% m(b% d_i)**2 - 1 ) * e2 - e3 ))
       ...
       do i = 2,b% anomaly_steps ! trapezoidal integration
          de = de + 0.5 * (b% edot_theta(i-1) + b% edot_theta(i)) * (b% time_co(i) - b% time_co(i-1)) 
       end do
